...
namespace QoL
{
    public static class SettingsOverride
    {
        ...
        public static void RemoveModuleOverride(string name)
        {
            ...
            if (wasOverwritten && value.HasValue)
            {
            ...
            if (hadOrig)
            {
                ...
                if (value.HasValue && (value.Value != orig))
                {
        ...
        public static void RemoveSettingOverride(string type, string field)
        {
            ...
            if (hadOrig)
            {
                ...
                if (enabled.HasValue && (enabled.Value != orig))
                {
        ...
        public static bool TryGetOrigSetting(string name, out bool value)
        {
            return _origSettings.TryGetValue(name, out value);
        }

        public static bool TryGetOrigModuleEnabled(string name, out bool value)
        {
            return _origEnabledModules.TryGetValue(name, out value);
        }
        ...
        static SettingsOverride()
        {
            ...
            _moduleOverrides = types.Where(t => t.IsSubclassOf(typeof(FauxMod)) && t.GetMethod(nameof(FauxMod.Unload)).DeclaringType != typeof(FauxMod))
                .Select(t => t.Name)
                .ToDictionary<string, string, bool?>(s => s, s => null);

            _fields = types.SelectMany(t => t.GetFields()).Where(f => f.FieldType == typeof(bool) && Attribute.IsDefined(f, typeof(SerializeToSetting)))
                .ToDictionary(f => $"{f.DeclaringType.Name}:{f.Name}");

            _settingOverrides = _fields.Keys.ToDictionary<string, string, bool?>(key => key, key => null);
        }
    }
}
